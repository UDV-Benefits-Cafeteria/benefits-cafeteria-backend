...
class Benefit(Base):
    ...
    products: Mapped[List["BenefitProduct"]] = relationship(
        "BenefitProduct", back_populates="benefit"
    )
...
class BenefitProduct(Base):
    ...
    content: Mapped[Text] = mapped_column(
        Text, nullable=True
    )  # что тут должно храниться?

    status: Mapped[BenefitStatus] = mapped_column(
        SQLAlchemyEnum(BenefitStatus), default=BenefitStatus.pending, index=True
    )
