...
@pytest.mark.asyncio
async def test_create_legal_entity_valid(hr_client1: AsyncClient):
    valid_legal_entity_data = {
        "name": "Test Entity",
    }
    response = await hr_client1.post("/legal-entities/", json=valid_legal_entity_data)
...
@pytest.mark.asyncio
async def test_create_legal_entity_invalid(hr_client1: AsyncClient):
    invalid_legal_entity_data = {
        "name": "",
    }
    response = await hr_client1.post("/legal-entities/", json=invalid_legal_entity_data)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


@pytest.mark.asyncio
async def test_update_legal_entity_not_found(hr_client1: AsyncClient):
    entity_id = 9999
    update_data = {
        "name": "Updated Name",
    }
    response = await hr_client1.patch(f"/legal-entities/{entity_id}", json=update_data)
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_update_legal_entity_valid(hr_client1: AsyncClient):
    legal_entity_data = {
        "name": "Original Entity",
    }
    create_response = await hr_client1.post("/legal-entities/", json=legal_entity_data)
...
@pytest.mark.asyncio
async def test_update_legal_entity_invalid(hr_client1: AsyncClient):
    legal_entity_data = {
...
@pytest.mark.asyncio
async def test_delete_legal_entity_not_found(hr_client1: AsyncClient):
    entity_id = 9999
    response = await hr_client1.delete(f"/legal-entities/{entity_id}")
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_delete_legal_entity_valid(hr_client1: AsyncClient):
    legal_entity_data = {
    ...
    delete_response = await hr_client1.delete(f"/legal-entities/{entity_id}")
    assert delete_response.status_code == status.HTTP_200_OK
    assert delete_response.json()["is_success"] is True

    get_response = await hr_client1.get(f"/legal-entities/{entity_id}")
